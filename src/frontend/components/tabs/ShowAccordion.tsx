...
export function ShowAccordion({ show }: { show: LocalShow }): JSX.Element {
    const [isAnimated, setAnimated] = React.useState<boolean>(false);

    return <div className="w-4/5 rounded" >
        {show ? (
            // <AccordionItem index={0} isAnimated={isAnimated} setAnimated={setAnimated}>First thing</AccordionItem>
            Object.keys(show.content).sort(compareAsNumbers).map((seasonNo: string) => <AccordionItem key={seasonNo} index={seasonNo} isAnimated={isAnimated} setAnimated={setAnimated} showSeason={(show.content as LocalShowContent)[seasonNo]}>Season {parseInt(seasonNo)}</AccordionItem>)
        ) :
            <Spinner />}
    </div>;
}

function AccordionItem({ index, children, isAnimated, setAnimated, showSeason }: { index: string, children: any, isAnimated: boolean, setAnimated: Function, showSeason: LocalSeason }): JSX.Element {
    ...
    return <div className="flex flex-col text-white overflow-hidden -my-px">
        ...
        <div ref={self} className={["w-full grid grid-cols-3 grid-flow-row-dense gap-4 p-4 dark:bg-slate-600", "flex", "flex-wrap", "justify-start", !isLoaded && "-translate-y-full"].join(' ')} style={{ transition: isAnimated ? `margin ${animationTimeMs / 1000.0}s linear` : 'margin 0s', marginBottom: (self.current && !openItems.has(index)) ? `-${offsetHeight}px` : '0px' }}>
            {Object.keys(showSeason).sort(compareAsNumbers).map((episodeNo: string, index: number) => <VideoCard key={episodeNo} gridCol={(index % 3) + 1} video={showSeason[episodeNo]} episodeNo={episodeNo}></VideoCard>)}
        </div>
    </div>
}