...
function AccordionItem({ index, children, isAnimated, setAnimated, showSeason, activeVideoPath, setActiveVideoByPath }: { index: string, children: any, isAnimated: boolean, setAnimated: Function, showSeason: LocalSeason, activeVideoPath: PathLike, setActiveVideoByPath: Function }): JSX.Element {
    ...
    return <div className="flex flex-col text-white overflow-hidden -my-px border-slate-400 select-none">
        <div className="cursor-pointer w-full flex justify-between items-center dark:bg-slate-800 p-6 z-10" onClick={onClickEvent}>
            <h1 className="font-medium text-2xl flex">{children}<h2 className="ml-4 font-extralight">({Object.keys(showSeason).length} episodes)</h2></h1>
	...
	s7.794-1.581,10.606-4.394l149.996-150C331.465,94.749,331.465,85.251,325.607,79.393z"/>
        ...
        <div ref={self} className={["w-full grid grid-cols-3 grid-flow-row-dense gap-4 p-8 dark:bg-slate-600", "flex", "flex-wrap", "justify-start", !isLoaded && "-translate-y-flll"].join(' ')} style={{ transition: isAnimated ? `margin ${animationTimeMs / 1000.0}s linear` : 'margin 0s', marginBottom: (self.current && !openItems.has(index)) ? `-${offsetHeight}px` : '0px' }}>
